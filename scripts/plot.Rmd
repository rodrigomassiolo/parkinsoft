...
datos <- datosenergy[,5:ncol(datosenergy)]
...
datos <- datosaudspec[,5:ncol(datosaudspec)]
resultadoAudspect <- expand.grid(Tiempo_segundos = seq(0,(nrow(datos)-1)/100,0.01),Espectro = seq(0,25,1))
resultadoAudspect$intensidad <- unlist(datos[,6:31],use.names=FALSE)
...
datos <- datosprosody[,6:ncol(datosprosody)]

#paso los datos a un data frame
resultado <- expand.grid(frame = seq(0,(nrow(datos)-1)/100,0.01))
resultado$F0_sma = unlist( datos$F0_sma)
resultado$voiceProb =unlist( datos$voiceProb_sma/1000000)
...
datos <- dbGetQuery(mydb,queryegemaps)
