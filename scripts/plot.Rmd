...
#CHROMA###########################################################################################################################################
if (parchroma == 1) {
querychoma <- paste0("SELECT * FROM CHROMA WHERE ejerciciopaciente_id = ", paej)
...
#ENERGY###########################################################################################################################################
if (parenergy == 1) {
queryenergy <- paste0("SELECT * FROM ENERGY WHERE ejerciciopaciente_id = ", paej)
datosenergy <- dbGetQuery(mydb,queryenergy)

datosenergy <- datosenergy[,5:ncol(datosenergy)]
...
energygraph <- ggplot(datos2,aes(x=tiempo,y=LogEnergy))+geom_line()+ geom_hline(yintercept=max(datos2$LogEnergy), linetype="dashed", color = "red") + 
  geom_hline(yintercept=min(datos2$LogEnergy), linetype="dashed", color = "blue") + geom_hline(yintercept=mean(datos2$LogEnergy), linetype="dashed", color = "green") + labs(title='Energia de la voz')
}
###################################################################################################################################################

#AUDSPEC###########################################################################################################################################
if (paraudspec == 1) {
queryaudspec <- paste0("SELECT * FROM AUDSPEC WHERE ejerciciopaciente_id = ", paej)
datosaudspec <- dbGetQuery(mydb,queryaudspec)
datosaudspec <- datosaudspec[,5:ncol(datosaudspec)]
resultadoAudspect <- expand.grid(Tiempo_segundos = seq(0,(nrow(datosaudspec)-1)/100,0.01),Espectro = seq(0,25,1))
...
#F0 con probabilidad
if (parprosody == 1) {
queryprosody <- paste0("SELECT * FROM PROSODY WHERE ejerciciopaciente_id = ", paej)
datosprosody <- dbGetQuery(mydb,queryprosody)
datosprosody <- datosprosody[,6:ncol(datosprosody)]

#paso los datos a un data frame
resultado <- expand.grid(frame = seq(0,(nrow(datosprosody)-1)/100,0.01))
resultado$F0_sma = unlist( datosprosody$F0_sma)
resultado$voiceProb =unlist( datosprosody$voiceProb_sma/1000000)
resultado$loudness = log(unlist( datosprosody$pcm_loudness_sma))
...
queryegemaps <- paste0("SELECT * FROM EGEMAPS WHERE ejerciciopaciente_id = ", paej)
datosegemap <- dbGetQuery(mydb,queryegemaps)
xs   <- seq(-2*pi,0,pi/100)
wave <- 0.5*sin(datosegemap$F1frequency_sma3nz_amean * xs) + 0.25*sin(datosegemap$F2frequency_sma3nz_amean * xs) + 0.125 * sin(datosegemap$F3frequency_sma3nz_amean * xs)
...
resumeneGemaps$Valor <- c(datosegemap$F0semitoneFrom27_5Hz_sma3nz_amean,datosegemap$F1frequency_sma3nz_amean,datosegemap$F2frequency_sma3nz_amean,datosegemap$F3frequency_sma3nz_amean,datosegemap$jitterLocal_sma3nz_amean,datosegemap$jitterLocal_sma3nz_stddevNorm,datosegemap$shimmerLocaldB_sma3nz_amean,datosegemap$shimmerLocaldB_sma3nz_stddevNorm)
...
#Grafico de vowel
vowelGraph <- ggplot(vowel,aes(x = f1)) + geom_point(aes(y = f2,color="vowel")) + geom_point(aes(x=datosegemap$F1frequency_sma3nz_amean,y=datosegemap$F2frequency_sma3nz_amean,color="paciente"))+
